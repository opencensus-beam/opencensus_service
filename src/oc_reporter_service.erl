...
report(Spans, _) ->
    ProtoSpans = [to_proto_map(Span) || Span <- Spans],
    oc_reporter_client:report_spans(ProtoSpans),
    ok.

to_proto_map(#message_event{type=Type,
                            id=Id,
    ...
    #{type => Type,
      ...
      uncompressed_size => UncompressedSize,
      compressed_size => CompressedSize};
to_proto_map(#status{code=Code,
                     message=Message}) ->
    #{code => Code,
      message => Message};
to_proto_map(#span{name=Name,
                   trace_id=TraceId,
                   span_id=SpanId,
...
to_std_stack_trace(Frames) ->
    #{stack_frames => #{frame => [to_std_stack_frame(F) || F <- Frames],
